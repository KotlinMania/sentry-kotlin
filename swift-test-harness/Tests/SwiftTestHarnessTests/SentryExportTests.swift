...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `Sentry.swiftmodule/`
...
//      `import Sentry` above would fail at compile time.
//
//   2. The static archive `libSentry.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "Sentry" }` and
//      `flattenPackage = "io.github.kotlinmania.sentry"` configuration in
...
final class SentryExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "Sentry swift module imported cleanly")
    }
}
