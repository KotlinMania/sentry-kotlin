# Claude Code Project Instructions — sentry-kotlin

## Project Overview

This is **sentry-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`sentry`](https://crates.io/crates/sentry).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/sentry/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/sentry>
